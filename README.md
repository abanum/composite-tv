...
| ドロップアウト / 補償 | 表示 | テープの信号欠落を時間軸で再現。長いものは次のラインへ折り返し、ノイズ帯＋直後の横ずれに。補償は なし / グレー / 1H遅延線（減衰・ぼやけつき） |
| テープ傷 / 位置 | 表示 | 折れ・縦傷による永久欠陥。毎フレーム同じ位置に出る横線。太さと縦位置を指定 |
...
| エンコーダ | I/Q 帯域, 変調クロマゲイン, 輪郭補正, アスペクト処理 | コンポジット化の帯域・色にじみ・エッジの残響 |
